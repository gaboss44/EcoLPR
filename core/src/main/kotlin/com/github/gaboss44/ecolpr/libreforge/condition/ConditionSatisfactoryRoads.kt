...
object ConditionSatisfactoryRoads : RoadCondition("satisfactory_roads") {
    override fun isMet(
        ...
        return when (mode) {
            EvaluationMode.ANY -> compileData.any { it.satisfies(player) }
            EvaluationMode.ALL -> compileData.all { it.satisfies(player) }
        }
    }
}